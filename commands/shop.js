...
module.exports = {
	...
	execute(message, args, msgUser, client, logger) {
		...
		Object.values(items).sort((a, b) => a.value - b.value).map((i) => {
			if (i.buyable) {
				if (i.ctg == 'consumable') consumable += `${i.emoji} ${i.name}: ${client.util.formatNumber(i.value)}💰\n`;
				else if (i.ctg == 'collectable') collectables += `${i.emoji} ${i.name}: ${client.util.formatNumber(i.value)}💰\n`;
