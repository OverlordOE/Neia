		...
		.addComponents(
			new StringSelectMenuBuilder()
				...
				.addOptions([
					{
						label: 'Balance',
						description: 'Highest Balance between all users.',
						value: 'balance',
					},
					{
						label: 'Numbers Counted List',
						description: 'Most Numbers Counted between all users.',
						value: 'count',
					},
					{
						label: 'Streaks Ruined List',
						description: 'Most Streaks Ruined between all users.',
						value: 'ruined',
					},
					{
						label: 'Money Gained Gambling List',
						description: 'Highest Money Gained Gambling between all users.',
						value: 'ggambling',
					},
					{
						label: 'Money Gained Counting List',
						description: 'Highest Money Gained Counting between all users.',
						value: 'gcounting',
					},
				]),
		);
		...
		if (i.user.id === interaction.user.id) {
			if (i.customId === 'previous' && page > 0) {
				...
				if (i.values[0] === 'balance') {
					listIndex = 0;
					menuRow.components[0].setPlaceholder('Balance');
					embed.setDescription(editDescription(listArray[listIndex], page));
					await i.update({ embeds: [embed], components: [menuRow, buttonRow] });
				}
					...
					listIndex = 3;
					menuRow.components[0].setPlaceholder('Money Gained Gambling');
					embed.setDescription(editDescription(listArray[listIndex], page));
					await i.update({ embeds: [embed], components: [menuRow, buttonRow] });
				}
				else if (i.values[0] === 'gcounting') {
					listIndex = 4;
					menuRow.components[0].setPlaceholder('Money Gained Counting');
					embed.setDescription(editDescription(listArray[listIndex], page));
					await i.update({ embeds: [embed], components: [menuRow, buttonRow] });
				}
			}
