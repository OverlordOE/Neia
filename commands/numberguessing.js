...
module.exports = {
	...
	async execute(message, args, msgUser, msgGuild, client, logger) {
		...
		const sentMessage = await message.channel.send(embed.setDescription('Guess the number between 1 and 5.').setTitle('Number Guessing'));
		...
		sentMessage.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] })
			.then(collected => {
				...
				if (reaction.emoji.name === client.emojiCharacters[answer]) {
					...
					sentMessage.edit(embed.setDescription(`You have chosen ${reaction.emoji.name}\nThe correct answer was ${client.emojiCharacters[answer]}.
					You win ${client.util.formatNumber(winAmount)}💰.\nYour current balance is ${client.util.formatNumber(balance)}💰`));
				}
				else {
